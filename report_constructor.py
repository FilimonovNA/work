...
def add_table_with_values(doc):
    ...
    doc.add_paragraph()
    table_adjusting(table, rows, cols)
    # table = doc.add_table(1, 2)
    # table.cell(0, 0).text = 'Left alignment'
    # table.cell(0, 0).paragraphs[0].paragraph_format.alignment = WD_TAB_ALIGNMENT.LEFT
    # table.cell(0, 1).text = 'Right alignment'
    # table.cell(0, 1).paragraphs[0].paragraph_format.alignment = WD_TAB_ALIGNMENT.RIGHT
    # table_adjusting(table)


def table_adjusting(tab, rows, cols):
...
def add_picture_caption(picture, doc, serial_number):
    ...
    '10': 'Функциональные показатели 3G indoor scanner', '11': 'Функциональные показатели LTE indoor scanner'
    }
...
if save_report(report_doc, report_path) != 1:
    ...
    for floor in all_floors:
        ...
        absolut_picture_number_in_file = construct_floor(report_doc, floor, floor_pictures_list,
                                                         absolut_picture_number_in_file)  # создаем этаж
